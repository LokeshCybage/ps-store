...
5. **TLS Termination**: Terminate TLS at Ingress/Gateway level
6. **Rate Limiting**: Apply rate limits at Ingress level
7. **Monitoring**: Expose metrics endpoints for Prometheus
8. **Gateway API**: Prefer Gateway API (`HTTPRoute`, `Gateway`) over legacy Ingress for new deployments
